...
-import(utils, [read_lines/1, transpose/1, partition_by_len/2]).

-export([solution_pt1/0, solution_pt2/0, parse_instruction/1]).

construct_stacks(ListOfStrings) ->
    lists:map(
        fun(LoL) -> lists:filter(fun(L) -> L /= [] end, LoL) end,
        transpose(
            lists:map(
                fun(L) -> lists:map(fun string:strip/1, L) end,
...
solution_pt1() ->
    ...
    Instructions = lists:map(
        fun parse_instruction/1, lists:nthtail(1, lists:dropwhile(fun(L) -> L /= "" end, Lines))
    ),

    lists:concat(
        lists:map(
            fun([H | _]) -> H end,
            lists:foldl(fun(X, Acc) -> move_stacks(X, Acc) end, Stacks, Instructions)
        )
    ).
...
solution_pt2() ->
    ...
    lists:concat(
        lists:map(
            fun([H | _]) -> H end,
            lists:foldl(fun(X, Acc) -> move_stacks_pt2(X, Acc) end, Stacks, Instructions)
        )
    ).
